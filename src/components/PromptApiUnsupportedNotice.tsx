...
import {
  MOBILE_DOWNLOAD_WARNING,
  builtinUnavailableReason,
  detectAiCapabilitySnapshot,
} from "../lib/aiCapability";

/**
 * 내장 AI를 쓸 수 없는 브라우저에서 회화/작문/선생님 페이지에 들어왔을 때의 인라인 안내.
 *
 ...
 * 떠밀고 있었다.
 *
 * 여기서는 이미 "못 쓴다"가 확정된 뒤라 `availability()`를 다시 기다릴 필요가 없다 —
 * 동기 스냅샷으로 충분하다(경로 판단이 아니라 문구 선택에만 쓴다).
 */
function PromptApiUnsupportedNotice({ feature }: { feature: string }) {
  const capability = detectAiCapabilitySnapshot();
  // 진짜 Chrome이면 플래그를 켜서 살릴 여지가 있다. 크로미움 포크는 켤 플래그조차 없다.
  const flagsMightHelp = capability.isGenuineChrome;

  if (capability.gemma) {
    return (
      <div className="m-4 rounded-2xl bg-info/10 p-5 text-sm text-gray-700">
        <p className="font-bold text-info">이 브라우저에서는 내장 AI를 쓸 수 없어요.</p>
        <p className="mt-2">{builtinUnavailableReason(capability)}</p>
        <p className="mt-2">
        ...
        {flagsMightHelp && (
          <p className="mt-3 text-gray-400">
            Chrome을 쓰고 계시다면 <ChromeLink path="flags" />에서 관련 플래그를 켜고 재시작해
            내장 AI를 살릴 수도 있어요.{" "}
            <Link to="/diagnostics" className="text-info">
              🩺 자가진단 →
            </Link>
          </p>
        )}
  ...
  return (
    <div className="m-4 rounded-2xl bg-warning/10 p-5 text-sm text-gray-700">
      ...
      <p className="mt-2">{builtinUnavailableReason(capability)}</p>
      <p className="mt-2">
        모델을 직접 받아 실행하는 방법도 쓸 수 없습니다(WebGPU 미지원). 다음 중 하나가 필요합니다:
      </p>
