 ...
 */
/**
 * 답변의 **모양**을 지시하는 줄. 모델은 이걸 그대로 소제목으로 옮겨 쓰곤 한다
 * (「② 문장을 단어·조사·활용으로 나눠 각각이 하는 역할」) — 시키는 대로 한 것이지 유출이 아니다.
 * 그래서 유출 검사 기준(`TEACHER_LEAK_REFERENCE`)에서는 이 줄들을 뺀다.
 */
const TEACHER_ANSWER_SHAPE = [
...
const TEACHER_RULES = [
  ...
  // **"앱이 따로 낸다"는 이유는 적지 않는다** — 모델이 그 말을 학습자에게 그대로 옮겨 하고,
  // 그게 유출 검사에 걸려 멀쩡한 답이 거절 문구로 바뀌었다(실제로 겪었다).
  "- 답변 끝에 연습 문제나 퀴즈를 덧붙이지 마세요.",
  "",
  "답변 형식:",
...
export const TEACHER_SYSTEM_PROMPT = [...TEACHER_INTRO, ...TEACHER_ANSWER_SHAPE, ...TEACHER_RULES].join("\n");

/**
 * `looksLikePromptLeak`에 넘길 기준 문자열 — 고정 지시문에서 **답변 모양 지시만 뺀 것**이다.
 * 모양 지시를 넣어두면 모델이 그걸 소제목으로 따라 쓴 정상 답변이 조각 두세 개로 걸려
 * 거절 문구로 바뀐다(「~할 것 같다」 질문에서 실제로 그랬다).
 */
