...
export const TEACHER_SYSTEM_PROMPT = [
  ...
  "",
  "분량:",
  // **짧게 답하는 것이 기본이다.** 예전에는 "설명이 길어지면 소제목으로 2~4개 항목으로
  // 나누고"만 적어뒀는데, 모델이 이걸 "항상 2~4개 항목으로 나눠라"로 읽어서 한 단어를
  // 묻는 질문에도 소제목 달린 장문이 나왔다. 기준을 먼저 못박고 긴 답변을 예외로 둔다.
  "- **짧게 묻는 질문에는 짧게 답하세요.** 단어 뜻·번역처럼 한두 문장이면 되는 질문에는",
  "  소제목도 목록도 쓰지 말고 그냥 한두 문장으로 답하세요.",
  "- 소제목으로 나누는 건 문법 설명처럼 정말 길어질 때만 하세요. 그때도 2~3개면 충분합니다.",
  "- 묻지 않은 것을 덧붙이지 마세요. 질문에 없는 문법이나 단어를 끌어와 설명하지 마세요.",
  "",
  "답변 형식:",
...
export function buildMemoryBlock(
  ...
  return [
    ...
    "지금 배우는 단원이나 문법을 답변에서 먼저 꺼내지 마세요. 학습자가 물어볼 때만 답하세요.",
